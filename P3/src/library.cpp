...
patron findPatron(const vector<patron>& patrons, int patronid) {
	patron p;
	p.patron_id = -1;
...
book findBook(const vector<book>& books, int bookid) {
	book b;
	b.book_id = -1;
...
void setPatron(const patron& patron) {
	for (int i = 0; i < patrons.size(); i++) {
		if (patrons[i].patron_id == patron.patron_id) {
			patrons[i] = patron;
			break;
		}
	}
...
void resave(const book& b, const patron& p) {
	setPatron(p);
	setBook(b);
...
int checkout(int bookid, int patronid){
	...
	patron p = findPatron(patrons, patronid);
...
int checkin(int bookid){
	reloadAllData();
	book b = findBook(books, bookid);
	patron p = findPatron(patrons, b.loaned_to_patron_id);
...
int enroll(std::string &name){
	...
	int maxId = -1;
	for (int i = 0; i < patrons.size(); i++) {
		if (patrons[i].patron_id > maxId) {
			maxId = patrons[i].patron_id;
		}
	}
	patron p;
	p.name = name;
